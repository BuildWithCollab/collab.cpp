...
class AreaState:
    ...
    # Lines belonging to top-level non-collab namespaces (std, fmt) — extracted
    # from decls_lines and injected into the cppm purview so importer TUs can
    # see specializations of templates from those namespaces. (MSVC modules
    # don't surface GMF-declared partial specializations of std/fmt templates
    # to importers; placing them in the purview fixes this.)
    purview_lines: list[str] = field(default_factory=list)
...
def split_routed_namespaces(decls_lines: list[str]) -> tuple[list[str], list[str]]:
    """Extract top-level `namespace std { ... }` and `namespace fmt { ... }`
    blocks from the parsed decls and return (remaining_decls, purview_lines).

    Specializations of templates declared in those namespaces (std::hash,
    std::formatter, fmt::formatter) must land in the cppm purview, not the GMF,
    so importer TUs find them via template specialization lookup."""
    ROUTED_NS = ("std", "fmt")
    out_decls: list[str] = []
    out_purview: list[str] = []
    in_routed = False
    brace_depth = 0
    for line in decls_lines:
        if not in_routed:
            stripped = line.strip()
            m = re.match(r"namespace\s+([A-Za-z_][A-Za-z0-9_]*)\s*\{\s*$", stripped)
            if m and m.group(1) in ROUTED_NS:
                in_routed = True
                brace_depth = 1
                out_purview.append(line)
                continue
            out_decls.append(line)
        else:
            out_purview.append(line)
            for c in _strip_strings_and_comments(line):
                if c == "{":
                    brace_depth += 1
                elif c == "}":
                    brace_depth -= 1
...
def emit_cppm(area: str, state: AreaState) -> str:
    ...
    for ns, names in by_ns.items():
        ...
        lines.append("}")
        lines.append("")
    # Routed namespace blocks (std/fmt specializations) live in the module
    # purview rather than the GMF so importer TUs find them via template
    # specialization lookup.
    if state.purview_lines:
        lines.extend(state.purview_lines)
    return "\n".join(lines).rstrip() + "\n"
...
def generate_area(area: str) -> None:
    ...
    state = parse(text)
    state.decls_lines, state.purview_lines = split_routed_namespaces(state.decls_lines)
